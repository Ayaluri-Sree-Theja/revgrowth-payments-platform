...
def _clean_value(v):
  # Convert NaN to None so JSON is valid (Postgres JSON does not allow NaN)
  if v is None:
    return None
...
def insert_billing_events_batch(events: Iterable[Dict[str, Any]], page_size: int = 5000) -> int:
